...
#         if (self.spy == False and self in team):
#             return True
                
#         if (self.spy == False and self.game.tries == 5):
#             return True
#         else:
#             return bool(self == self.game.leader)
...
class CountingBot(Bot):
    def select(self, players, count):
        ...
        if self.N_failed_missions != {}:
            ...
            keysP = self.N_failed_missions.keys()
            values = list(keysP)
            ...
            for i in values:
                if (i!=self):
                    not_me_in_team.append(i)

            while cont < count-1:
                pList.append(not_me_in_team[cont])
                cont +=1

            return pList

        else:
            pList= self.others()[:count-1]
            pList.append(self)
            return pList# TODO change this

    def vote(self, team):

        if self.spy:
            return False 
        else:     #maybe order the new list ?
            N_failed_missions = sorted(self.failed_missions_been_on.values()) 
            self.N_failed_missions = {}

            for i in N_failed_missions:
                for j in self.failed_missions_been_on.keys():
                    if (self.failed_missions_been_on[j]==i):
                        self.N_failed_missions[j] = self.failed_missions_been_on[j]

            first_suspect = (list(self.N_failed_missions.items())[-1])
            second_suspect = (list(self.N_failed_missions.items())[-2])


            if ((first_suspect in team and self!= first_suspect)  or (second_suspect in team and self != second_suspect)):
                return False 
            else:
                return True
    ...
    def onGameRevealed(self, players, spies):
        ...
        for i in players:
            self.failed_missions_been_on[i]=0
